...
//carregar os clubes ao abrir a página
buscarClubesAwait(); 

// /* BUSCAR XMLHttpRequest */
function buscarClubes() { //método mais antigo para implementar o AJAX
...
/* BUSCAR async await */
async function buscarClubesAwait() { //método moderno da implementação do AJAX
    const options = {
        method: "GET"
    };
...
async function excluirClube(idClube) {
    ...
    if(resp.status == 201){ //tratar a resposta
